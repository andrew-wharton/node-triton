 ...
 *      - {String} name: Required. A package name, short id, or id (a UUID).
 *      - {Boolean} wait: Wait (via polling) until the resize is complete.
...
TritonApi.prototype.resizeInstance = function resizeInstance(opts, cb) {
    ...
    vasync.pipeline({arg: {client: self, id: opts.id, package: opts.package}, funcs: [
        _stepInstId,

        _stepPkgId, // should this be done in parallel using vasync.parallel?
